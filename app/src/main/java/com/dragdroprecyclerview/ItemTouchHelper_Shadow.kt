 ...
 * [androidx.recyclerview.widget.ItemTouchHelper.Callback.onMove] and / or
 * [androidx.recyclerview.widget.ItemTouchHelper.Callback.onSwiped].
 *
 *
 ...
 * [androidx.recyclerview.widget.ItemTouchHelper.Callback] class or implementing [androidx.recyclerview.widget.ItemTouchHelper.ViewDropHandler]
 * interface in your LayoutManager.
 *
 *
 * By default, ItemTouchHelper moves the items' translateX/Y properties to reposition them. You can
 * customize these behaviors by overriding [androidx.recyclerview.widget.ItemTouchHelper.Callback.onChildDraw]
 * or [androidx.recyclerview.widget.ItemTouchHelper.Callback.onChildDrawOver].
 *
 *
 * Most of the time you only need to override `onChildDraw`.
 */
class ItemTouchHelper_Shadow
/**
 ...
 */(
     ...
     */
    var mSelected: /* synthetic access */ViewHolder? = null
     ...
     */
    var mActivePointerId/* synthetic access */ = ACTIVE_POINTER_ID_NONE
     ...
     * [ItemTouchHelper_Shadow.Callback.getAbsoluteMovementFlags] for the current
     * action state.
     */
    ...
    fun  /* synthetic access */postDispatchSwipe(anim: RecoverAnimation, swipeDir: Int) {
        // wait until animations are complete.
        mRecyclerView!!.post(object : Runnable {
            override fun run() {
                if (mRecyclerView != null && mRecyclerView!!.isAttachedToWindow
                    && !anim.mOverridden
                    && (anim.mViewHolder.absoluteAdapterPosition
     ...
     */
    fun  /* synthetic access */moveIfNecessary(viewHolder: ViewHolder) {
     ...
     * [ItemTouchHelper_Shadow.Callback.isLongPressDragEnabled].
     *
     *
     ...
     * ItemTouchHelper
     * is attached.
     *  * [ItemTouchHelper_Shadow.Callback] must have dragging enabled.
     ...
     * RecyclerView.
     * @see ItemTouchHelper_Shadow.Callback.isItemViewSwipeEnabled
     */
     ...
     * behavior
     * by overriding [ItemTouchHelper_Shadow.Callback]
     *
     *
     * For this method to work:
     *
     ...
     *  * [ItemTouchHelper_Shadow.Callback] must have swiping enabled.
     *  * There must be a previous touch event that was reported to the ItemTouchHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.
     *
    ...
    fun  /* synthetic access */findAnimation(event: MotionEvent): RecoverAnimation? {
    ...
    fun  /* synthetic access */updateDxDy(ev: MotionEvent, directionFlags: Int, pointerIndex: Int) {
     ...
     * [ItemTouchHelper_Shadow].
     */
    interface ViewDropHandler {
        /**
         * Called by the [ItemTouchHelper_Shadow] after a View is dropped over another View.
         *
         *
     ...
     * [ItemTouchHelper_Shadow.SimpleCallback].
     *
     *
     * If user drags an item, ItemTouchHelper will call
     * [ onMove(recyclerView, dragged, target)][ItemTouchHelper_Shadow.Callback.onMove].
...
//        val isLongPressDragEnabled: Boolean = true
        ...
        fun chooseDropTarget(
            ...
            for (i in 0 until targetsSize) {
                ...
                if (dy < 0) {
                    // Change sensitivity with 0.7
                    val diff = (target.itemView.top + selected.itemView.height * 0.7 - curY).toInt()
                ...
                if (dy > 0) {
                    // Change sensitivity with 0.7
                    val diff =
                        (target.itemView.bottom - selected.itemView.height * 0.7 - bottom).toInt()
         ...
         * it is cleared.
         * @param actionState One of [ItemTouchHelper_Shadow.ACTION_STATE_IDLE],
         * [ItemTouchHelper_Shadow.ACTION_STATE_SWIPE] or
         * [ItemTouchHelper_Shadow.ACTION_STATE_DRAG].
         * @see .clearView
         */
        open fun onSelectedChanged(viewHolder: ViewHolder?, actionState: Int) {
            if (viewHolder != null) {
                ItemTouchUtilImpl.INSTANCE.onSelected(viewHolder.itemView)
            }
        }
        ...
        open fun clearView(recyclerView: RecyclerView, viewHolder: ViewHolder) {
            ItemTouchUtilImpl.INSTANCE.clearView(viewHolder.itemView)
        }
        ...
        fun onChildDraw(
            ...
            ItemTouchUtilImpl.INSTANCE.onDraw(
        ...
        fun onChildDrawOver(
            ...
            ItemTouchUtilImpl.INSTANCE.onDrawOver(
                c, recyclerView, viewHolder.itemView, dX, dY,
                actionState, isCurrentlyActive
            )
        }
        ...
        companion object {
            ...
            const val RELATIVE_DIR_FLAGS = (START or END
                    or (START or END shl DIRECTION_FLAG_COUNT)
                    or (START or END shl 2 * DIRECTION_FLAG_COUNT))
            private const val ABS_HORIZONTAL_DIR_FLAGS = (LEFT or RIGHT
            ...
            /**
             * Returns the [ItemTouchUIUtil] that is used by the [ItemTouchHelper_Shadow.Callback] class for
             * visual
             ...
             *
             *
             * By default, [ItemTouchHelper_Shadow.Callback] applies these changes on
             ...
             *
             * @return The [ItemTouchUIUtil] instance that is used by the [ItemTouchHelper_Shadow.Callback]
             */
            val defaultUIUtil: ItemTouchUIUtil
                get() = ItemTouchUtilImpl.INSTANCE
    ...
    private inner class ItemTouchHelperGestureListener internal constructor() :
         ...
         * [ItemTouchHelper_Shadow.ItemTouchHelperGestureListener.onLongPress].
         *
         ...
         * [ItemTouchHelper_Shadow.ItemTouchHelperGestureListener.onLongPress] being called.
         */
